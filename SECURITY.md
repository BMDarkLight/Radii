...
| Certificate revocation | `radii-proto::tls` (`crl`) | An optional PEM CRL per `[tls]`-style section, checked against the peer's leaf certificate in **both** directions — a revoked node is refused whether it presents the certificate as a client or as a server. End-entity only, because a private mesh CA has no issuer above it to publish its own status. A configured-but-unreadable CRL fails to load rather than silently revoking nothing. Read at startup, and only enforced by nodes that load it, so revocation is as fast as your rollout. See [`docs/tls.md`](docs/tls.md) |
| Graph snapshot is always sendable | `radii-crawl` (`graph_snapshot`), `radii-proto` (`SNAPSHOT_BUDGET`) | A `GraphSnapshot` is filled against a byte budget, not just entry counts, so it can never exceed `MAX_FRAME_LEN`. It previously could: the per-peer cap bounded how many observations a peer held, never how large each was, and one peer inside every quota could make `write_message` refuse the frame — failing every `GraphQuery` from every Head and Fetch poller in the mesh, for as long as those entries lived. Overflow now truncates and logs the dropped count, because routing from a partial view is survivable and routing from no view is not |
| Wire bounds on peer-supplied strings | `radii-proto` (`validate_message`) | Every string and list on a decoded message is bounded, in the direct *and* Head-relayed shapes, and on `GraphSnapshot` too (so a hostile Crawl cannot push unbounded strings into a poller's planner). `MAX_NODE_ID_LEN` covers `node_id` and a report's `from`/`target` — `target` matters most, since `authorized()` checks only `from` against the peer's certificate |
| Health reflects subsystem state | `radii-head` (`/health`) | Answers 503, not 200, when the graph poller has gone stale (no successful query in `3 ×` the poll interval) or has never succeeded. The protocol registry already takes the process down when a runner *fails*; a runner that is merely *degraded* — a Crawl outage, which the poller logs and retries by design — was previously invisible to a load balancer. The JSON body names the reason |
...
| A relay certificate is a bandwidth grant | With admission by CA membership, issuing a certificate entitles the holder to forwarding capacity, not only to writing graph reports. Narrow with `allow_peers` where that is not intended, and revoke with a `crl` (see [`docs/tls.md`](docs/tls.md)) to withdraw one without rotating the CA. A CRL only affects nodes that load it, and is read at startup — so revocation is as fast as your rollout, not instant |
...
| Graph state is bounded, but the bounds are the only defence | Crawl's reachability table is keyed (a repeat report replaces rather than appends), capped globally (`MAX_REACHABILITY_ENTRIES` = 16384) and per peer (`MAX_REACHABILITY_ENTRIES_PER_PEER` = 1024), aged out by `node_ttl_ms`, and now bounded in entry *size* as well as count (`MAX_NODE_ID_LEN` = 256, `MAX_PROTOCOL_LEN` = 64, `MAX_NODE_ROLES` = 16) — the count caps alone left `target`/`protocol` unbounded, and `authorized()` constrains only `from`. Planning is capped by `MAX_GRAPH_NODES` / `MAX_GRAPH_LINKS` / `MAX_ROUTE_HOPS` / `MAX_ROUTE_RESULTS`. A peer can still fill its own per-peer share with invented targets, and a graph truncated at a cap means every planner works from a partial view — `GraphSnapshot::dropped_links()` is non-zero when that happens and Head logs it |
| Crawl's node registry is unbounded and never evicted | `CrawlState::nodes` has no global or per-peer cap, and `node_ttl_ms` only filters expired nodes out of `GraphQuery` replies — nothing removes them from memory. On a plaintext listener a peer can register unlimited distinct node ids and the memory is never reclaimed; mTLS bounds it to one entry per authenticated identity, since `authorized()` requires a hello's `node_id` to match the peer's certificate. Entry size is bounded (see above); entry count is not |
...
- Peer-supplied strings and lists inside a frame are bounded too, in both the direct and Head-relayed shapes — a legal frame size is not on its own a bound on what the frame makes a listener *retain*.
- A `GraphSnapshot` is built against a byte budget so a reply can never exceed the frame limit; overflow truncates and is logged rather than failing the query.
- Callers must still apply timeouts; a client can stall after advertising a legal length.
- Future work: per-connection read timeouts and max concurrent connections outside the relay path, and a cap on Crawl's node registry.

### Crawl memory growth

Reachability reports are keyed, capped globally and per peer, aged out by `node_ttl_ms`, and bounded in entry size — see the graph-state row in the gap table above for the exact constants and what remains.

The **node registry** is the part still unbounded: `CrawlState::nodes` has no cap, and `node_ttl_ms` only hides expired nodes from `GraphQuery` replies rather than removing them. On a plaintext listener a peer can register unlimited distinct node ids permanently. Enable mTLS, which limits a peer to one registry entry (its own authenticated id), or restrict who can reach the listener.

...
- Configurable node liveness (`node_ttl_ms`, defaults to 60000ms / 60s): a node that stops sending hellos drops out of `GraphQuery` replies once its TTL elapses — long-lived participants are expected to re-send `NodeHello` periodically, well under the TTL, to stay visible
